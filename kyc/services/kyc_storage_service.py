...
def _validate_image_upload(file: UploadedFile) -> tuple[bytes, str]:
    ...
    # Requirement: only images. We keep it strict to JPEG to match the required .jpg path format.
    if normalized not in ALLOWED_IMAGE_CONTENT_TYPES:
        raise KycFileValidationError("Invalid file type (only JPEG images allowed)")
...
def upload_file(file: UploadedFile, rider_id: str, document_type: str) -> str:
    ...
    object_path = f"kyc/{safe_rider_id}/{safe_doc_type}_{_now_ms()}.jpg"
